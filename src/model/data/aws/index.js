...
async function deleteFragment(ownerId, id) {
  //Delete Data
  //We will need bucket name and key as before
  const params = {
    Bucket: process.env.AWS_S3_BUCKET_NAME,
    // Our key will be a mix of the ownerID and fragment id, written as a path
    Key: `${ownerId}/${id}`,
  ...
  const dynamoCommand = new DeleteCommand(paramsDynamo);
  ...
  try {
    ...
    logger.error(
      { err, Bucket, Key },
      'Error deleting fragment data from S3 or metadata from Dynamo'
    );
    throw new Error('unable to delete fragment data or metadata');
  }
